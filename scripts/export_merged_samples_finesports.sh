...
usage() {
  cat <<'USAGE'
Usage:
  scripts/export_merged_samples_finesports.sh \
...
Notes:
  ...
  - 先导出到 output/json/FineSports，再平铺合并到 output/merged
USAGE
}
...
if [[ -z "$FINE_ROOT" ]]; then usage; exit 1; fi
...
echo "Done. Exported FineSports to $OUT_JSON and merged to $OUT_MERGED"
