    ...
    return [
        {
            "role": "user",
            "content": [
                {"type": "image", "image": "/storage/wangxinxing/code/action_data_analysis/data/MultiSports_step1_first_frame/example/1.jpg"},
                {"type": "image", "image": "/storage/wangxinxing/code/action_data_analysis/data/MultiSports_step1_first_frame/example/0.jpg"},
...
def infer_batch(
    ...
    messages_list = [build_messages(p) for p in image_paths]
    texts = [
    ...
    return [parse_label_from_output(t) for t in output_texts]
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=8, dest="batch_size", help="每批次推理的图片数量")
    ...
    with open(output_txt, "w", encoding="utf-8") as fout:
        with tqdm(total=total, desc="处理图片", unit="张") as pbar:
            for start in range(0, total, batch_size):
                batch = items[start:start + batch_size]
                file_ids = [fid for fid, _ in batch]
                image_paths = [p for _, p in batch]
                try:
                    labels = infer_batch(
                        model=model,
                        processor=processor,
                    ...
                    # Fail-safe: record 0 for the whole batch
                    print(f"[WARN] 批量推理失败: {start}-{start + len(batch) - 1} -> {e}", file=sys.stderr)
                    labels = [0] * len(batch)
                for fid, label in zip(file_ids, labels):
                    fout.write(f"{fid},{label}\n")
